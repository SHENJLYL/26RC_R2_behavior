...
但本包不是 PB2025 的拷贝，也不运行 PB2025 代码：

- 不依赖 `pb2025_sentry_behavior`。
- 不复制 PB2025 中 R2 暂时无关的裁判系统、自瞄、云台、RFID 等节点。
- 只保留 R2 当前需要的最小运行框架和通用接口。
- 后续新增能力优先复用现有 R2/Sakiko 的 topic、service、action。
...
| 急停判断 | `IsEmergencyStopRequestedPlaceholder` | `/r2/emergency_stop`，同时保留底层独立急停 |
...
完整流程验证树用于不依赖导航、机械臂、视觉的情况下验证比赛流程顺序：
